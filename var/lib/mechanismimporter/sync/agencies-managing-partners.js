...
//
function removeParterFromAgencyManagement(countryName, partnerCode, partnerName, partnerManagedGroupNames, partnerShareGroupNames, allowedManagerNames) {
    ...
    for (var i in partnerShareGroupNames) {
        var groupName = partnerManagedGroupNames[i];
        var group = dhis.getCache("userGroup", "name", groupName);
        if (group) {
            var sharers = group.userGroupAccesses;
            if (sharers) {
                for (s in sharers) {
                    if (sharers.hasOwnProperty(s)) {
                        log.trace("removeParterFromAgencyManagement sharer " + util.inspect(sharers[s]));
                        var sharerName = sharers[s].displayName;
                        if (( sharerName.search(/^Global Agency .* user administrators$/) == 0
                            || sharerName.search(/^OU .* Agency .* user administrators$/) == 0 )
                            && ( !allowedManagerNames || allowedManagerNames.indexOf(sharerName) < 0)) {
                            log.action("Unsharing partner group '" + groupName + "' from '" + managerName + "'");
                            dhis.unshareIfExists("userGroup", group, sharers[s]);
                        }
                    }
                }
            }
...
//
function assignAgencyToManagePartner(countryName, partnerCode, partnerName, partnerManagedGroupNames, partnerShareGroupNames, agencyName, partnerGroupName, managers) {
    ...
    dhis.shareCached("userGroup", partnerShareGroupNames, '--------', agencyGroupName );

    removeParterFromAgencyManagement(countryName, partnerCode, partnerName, partnerManagedGroupNames, partnerShareGroupNames, [agencyGroupName /*, globalAgencyGroupName */])
}
                ...
                if (partnerCodes.hasOwnProperty(partnerCode)) {
                    ...
                    if (!partnerGroup || !partnerGroup.id) {
                        ...
                        if (agencies.length == 1) {
                            assignAgencyToManagePartner(countryName, partnerCode, partnerName, partnerManagedGroupNames, partnerShareGroupNames, agencies[0], managers);
                        } else {
                            removeParterFromAgencyManagement(countryName, partnerCode, partnerName, partnerManagedGroupNames, partnerShareGroupNames);
                        }
                    }
                }
            }
